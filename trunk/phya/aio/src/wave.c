...
//	////printf("return from alloc mem %x\n",  

	   GlobalAlloc(GMEM_FIXED, sizeof(WAVEFORMATEX)+cbExtraAlloc);
			...
			)
{
	...
	*cbSize = 0;
	...
	if ((nError = WaveOpenFile(pszFileName, &hmmioIn, ppwfxInfo, &ckInRiff)) != 0)
		{
////printf("mem err in OpenFile ***\n");
		goto ERROR_LOADING;
		}

	if ((nError = WaveStartDataRead(&hmmioIn, &ckIn, &ckInRiff)) != 0)
		{
////printf("mem err in DataRead\n");
		goto ERROR_LOADING;
		}
